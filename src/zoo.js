...
function animalCount(species) {
  ...
  if (!species) {
    animals.forEach((e) => {
      result[e.name] = e.residents.length;
      return result;
    });
    return result;
  }
...
function oldestFromFirstSpecies(id) {
  ...
  const animal = animals.find(as => as.id === responsable.responsibleFor[0]);

  animal.residents.forEach((selectedage) => {
    if (selectedage.age > maior) {
      maior = selectedage.age;
    }
    return maior;
  });
...
function employeeCoverage(idOrName) {
  ...
  if (!idOrName) {
    employees.forEach((selectEmp) => {
      ...
      result[`${selectEmp.firstName} ${selectEmp.lastName}`] = animalsName;
    });
    return result;
  }
  ...
  employe.responsibleFor.forEach((specieidrespon) => {
    const selectedAnimal = animals.find(selectAnm => selectAnm.id === specieidrespon).name;
    animalsRespon.push(selectedAnimal);
